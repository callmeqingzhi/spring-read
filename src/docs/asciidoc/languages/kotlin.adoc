...
----
	Flux<bean.User> users  = client.get().retrieve().bodyToFlux(bean.User.class)
----
...
----
	val users = client.get().retrieve().bodyToFlux<bean.User>()
	// or (both are equivalent)
	val users : Flux<bean.User> = client.get().retrieve().bodyToFlux()
----
...
* toString() of the form "bean.User(name=John, age=42)"
